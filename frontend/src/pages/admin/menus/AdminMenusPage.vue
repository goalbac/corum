        ...
        <template v-if="form.menuType !== 'GROUP'">
          ...
          <template v-else>
            ...
            <el-form-item v-if="!form.urlAuto || form.menuType === 'LINK'" label="URL">
              <el-input v-model="form.url" :placeholder="form.menuType === 'LINK' ? 'https://...' : 'notice 또는 news/notice'" />
              <div v-if="form.menuType === 'PAGE'" class="form-hint" style="margin-top:5px">
                예: news/notice 입력 시 사이트주소/news/notice 로 접속됩니다. login·admin·board·menu 등 시스템 경로로 시작하거나, 다른 메뉴와 중복되는 URL은 사용할 수 없습니다.
              </div>
            </el-form-item>
          </template>
        </template>
